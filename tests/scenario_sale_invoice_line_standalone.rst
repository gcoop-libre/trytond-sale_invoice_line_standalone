...
Create sale and manual invoice::
    ...
    'processing'
    >>> sale.reload()
    >>> assertEqual(len(sale.moves), 1)
    >>> assertEqual(len(sale.shipment_returns), 0)
    >>> assertEqual(len(sale.invoices), 0)
    >>> assertEqual(len(sale.invoice_lines), 0)
    >>> assertEqual(len(sale.shipments), 1)
...
Done shipment::
    ...
    >>> shipment.click('pack')
    >>> shipment.click('do')
    >>> assertEqual(shipment.state, 'done')
    >>> config.user = sale_user.id
    >>> sale.reload()
    >>> assertEqual(sale.state, 'processing')
    >>> assertEqual(len(sale.invoice_lines), 0)
...
Create manual invoices (invoice lines)::
    ...
    >>> assertEqual(len(sale.invoices), 0)
    >>> assertEqual(len(sale.invoice_lines), 1)
